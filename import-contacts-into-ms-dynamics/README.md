...
The application accepts a CSV file by polling a local folder frequently using a scheduler component. The CSV file contains contact information with columns as follows: first name, last name, phone number and email. These columns are mapped to each of the respective fields in a specific Dynamics CRM account and the rows are uploaded.
...
1. In your application in Studio, click the *mule.artifact.properties* file
...
1 Also set Authentication Retries, path to the *contacts.csv* working directory and scheduler parameters, e.g.:
...
1. The Application checks frequently the input folder for files using a scheduler component. It picks up the CSV file and processes it.
